...
<%-- 请求调用CustomerServlet的edit方法，这个方法根据cid进行更改本用户的信息 --%>
...
<table border="0" align="center" width="40%" style="margin-left: 100px;">
	<tr>
		...
		<td width="40%">
			<input type="text" name="cname" value="${requestScope.c.cname }"/>
		</td>
		<td align="left">
	...
	<tr>
		<td>客户性别</td>
		<td>
			<%-- <c:if test="${c.gender eq '男'}">checked='checked'</c:if> 判断转发过来的客户对象是不是男的，是就默认选择这个选项 --%>
			<input type="radio" name="gender" value="男" id="male" <c:if test="${c.gender eq '男'}">checked='checked'</c:if>>
			<label for="male">男</label>
			<input type="radio" name="gender" value="女" id="female" <c:if test="${c.gender eq '女'}">checked='checked'</c:if>/>
	...
	<tr>
		<td>客户生日</td>
		<td>
			<input type="text" name="birthday" id="birthday" readonly="readonly" value="${c.birthday }"/>
		</td>
		<td>
	...
	<tr>
		<td>描述</td>
		<td>
			<textarea rows="5" cols="30" name="description">${c.description }</textarea>
		</td>
		<td>
