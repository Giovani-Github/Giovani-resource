...
class DownLoadThread extends Thread {
	...
	public void run() {
		...
		File file = new File(threadId + ".txt"); //记录改线程下载位置的临时文件
		...
		try {
			...
			// 8. 如果临时文件存在，就从中取出上次下载的总字节数，计算出本次下载的开始位置
			if(file.exists()) { 
...
System.out.println(threadId + "下载的位置：" + startIndex + "-" + endIndex);			
			...
			if(conn.getResponseCode() == 206) {
				...
				while((len = is.read(b)) != -1) {
					...
					// 7. 同步把当前读取到的总字节数写入临时文件中
					RandomAccessFile indexFile = new RandomAccessFile(file, "rwd"); // 进度临时文件保存的位置
					indexFile.write((startIndex + total + "").getBytes()); //本次下载的开始位置 + 本次下载到的总字节数 = 下次开始下载的位置
					indexFile.close();
				}
				raf.close();
				MultiThreadDownload.finishedThread++; // 如果有一条线程下载完毕，就加1
				
				// 9. 删除记录下载位置的临时文件
				synchronized (this) { //线程同步
					if(MultiThreadDownload.finishedThread == MultiThreadDownload.threadCount) { // 如果三条线程都完成了下载
						for(int i = 0; i < MultiThreadDownload.threadCount; i++) {
							File f = new File(i + ".txt");
							f.delete();
						}
						MultiThreadDownload.finishedThread = 0;
					}
				}
